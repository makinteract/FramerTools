var bar, bg, cards, data, home, loadTable, m, nav, showSnackBar, status;
...
nav.back.on(Events.TouchEnd, function() {
  var c, j, len;
  for (j = 0, len = cards.length; j < len; j++) {
    c = cards[j];
    c.ignoreEvents = false;
  }
  return bar.ignoreEvents = false;
});
...
bar.tabs["trending"].on(Events.TouchEnd, function() {
  var snack;
  if (showSnackBar === false) {
    snack = new m.SnackBar({
      text: "Other tabs are not built out"
    });
    showSnackBar = true;
...
bar.tabs["subscriptions"].on(Events.TouchEnd, function() {
  var snack;
  if (showSnackBar === false) {
    snack = new m.SnackBar({
      text: "Other tabs are not built out"
...
bar.tabs["account"].on(Events.TouchEnd, function() {
  var snack;
  if (showSnackBar === false) {
    snack = new m.SnackBar({
      text: "Other tabs are not built out"
...
loadTable = function(table, superLayer) {
  var button, card, i, j, post;
  card = function() {
    var layer, profilePhoto, thumbnail, titleLabel, userLabel;
    ...
    Utils.delay(.3 * (layer.index - 1), function() {
    ...
    layer.on(Events.Click, function() {
      var authorTitle, bottomDivider, c, details, hideDetails, j, len, profileIcon, shareIcon, showDetails, subs, thumbsDownCount, thumbsDownIcon, thumbsUpCount, thumbsUpIcon, topDivider, video, videoTitle, videoViewCount, view;
      for (j = 0, len = cards.length; j < len; j++) {
        c = cards[j];
        c.ignoreEvents = true;
      }
      ...
      thumbsUpCount = new m.Text({
        ...
        constraints: {
          verticalCenter: thumbsUpIcon,
          leading: [thumbsUpIcon, 10]
        },
        fontSize: 14,
        superLayer: details
      });
      ...
      thumbsDownCount = new m.Text({
        ...
        constraints: {
          verticalCenter: thumbsDownIcon,
          leading: [thumbsDownIcon, 10]
        },
        fontSize: 14,
        superLayer: details
      });
      shareIcon = new m.Icon({
        ...
        constraints: {
          verticalCenter: thumbsUpIcon,
          leading: [thumbsDownCount, 20]
        }
      });
      topDivider = new Layer({
        superLayer: details,
        backgroundColor: m.color("grey200")
      });
      ...
      authorTitle = new m.Text({
        constraints: {
          topEdges: profileIcon,
          leading: [profileIcon, 10]
        },
        text: layer.data.author,
        superLayer: details
      });
      return subs = new m.Text({
        constraints: {
          top: [authorTitle, 5],
          leading: [profileIcon, 10]
        },
